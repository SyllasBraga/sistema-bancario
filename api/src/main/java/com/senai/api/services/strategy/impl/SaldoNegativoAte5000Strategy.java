...
@Component
public class SaldoNegativoAte5000Strategy implements ValidacaoSaldoStrategy {

    @Override
    public void validarSaldo(ContaModel conta, AcaoEnum acao, BigDecimal valor) {
        if (acao == AcaoEnum.RETIRAR && conta.getSaldo().subtract(valor).compareTo(new BigDecimal("-5000")) < 0) {
