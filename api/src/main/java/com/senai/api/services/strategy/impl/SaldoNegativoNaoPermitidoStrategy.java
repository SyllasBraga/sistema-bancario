...
@Component
public class SaldoNegativoNaoPermitidoStrategy implements ValidacaoSaldoStrategy {

    @Override
    public void validarSaldo(ContaModel conta, AcaoEnum acao, BigDecimal valor) {
        if (acao == AcaoEnum.RETIRAR && conta.getSaldo().subtract(valor).compareTo(BigDecimal.ZERO) < 0) {
